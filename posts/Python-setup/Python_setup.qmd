...
Python 웹사이트에서 제공하는 공식문서(<https://docs.python.org/3/using/index.html>)를 참조하는 것이 이상적이겠지만, 아래와 같이 간단한 안내에 따라 설치하셔도 됩니다.
...
윈도우에 처음 설치하는 경우에는 연구회에서는 Install Now 대신 Customize installation 선택을 추천합니다 (@fig-CustomizeInstallation). 이는 설치경로를 우리가 원하는데로 `C:\Python\Python-x.y.z`로 설정할 수 있는 장점이 있습니다(Command line interface 에서 경로를 입력할 때를 가정하면 짧고 직관적인게 좋습니다.).
...
For first-time installations on Windows, it is advisable to choose Customized installation instead of Install Now (@fig-CustomizeInstallation). This option allows you to set the installation path to `C:\Python\Python-x.y.z` as desired. (Assuming that you prefer short and intuitive paths when entering paths in the command line interface.)
