...
GitHub는 버전 관리 시스템인 Git을 기반으로 하여 코드의 변경 내역을 쉽게 추적하고 관리할 수 있게 해줍니다. 이를 통해 프로젝트의 과거 버전으로 쉽게 돌아갈 수 있으며, 여러 개발자가 동시에 작업할 때 발생할 수 있는 충돌을 효율적으로 해결할 수 있습니다. 여러 개발자가 동일한 프로젝트에서 작업할 수 있도록 브랜치를 만들어 독립적으로 개발한 후, Pull Request를 통해 코드를 병합할 수 있습니다. GitHub는 또한 프로젝트 관리를 위한 다양한 도구를 제공합니다. 이슈 트래커(issue tracker)를 통해 버그와 기능 개선 사항을 관리할 수 있고, 프로젝트 보드(project board)를 사용하여 작업 흐름을 시각적으로 관리할 수 있습니다. R로 통계 프로젝트를 할 때, git 설치가 반드시 필요한 것은 아닙니다. 하지만 제가 만든 예제 R프로젝트가 github 원격저장소에 보관되어 있으므로 git의 설치가 추천됩니다. 만약에 git의 사용이 어려우신 분들은 github 원격저장소를 클라우드처럼 생각하시고 필요한 파일들을 직접 하나씩 다운로드 받으셔도 됩니다.
...
예제 R프로젝트를 github에 원격저장소에 보관 중이며 URL은 아래와 같습니다.

```{r github_remote, eval=FALSE, filename="원격저장소 URL"}
https://github.com/RPythonStudy/Website.git
```

```{r eval=FALSE, filename="uer.email 설정예시"}
git config --global user.name "kimbi.kirams@gmail.com"}
```
